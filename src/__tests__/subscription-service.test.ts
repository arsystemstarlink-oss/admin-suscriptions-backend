...
describe('SubscriptionBusinessService', () => {
  ...
  describe('createSubscription (retroactiva)', () => {
    it('debería crear suscripción retroactiva con períodos históricos pagados', () => {
      ...
      const historicalPayments: HistoricalPaymentInput[] = [
        {
          ...
          startDate: new Date(Date.UTC(2026, 4, 5)),
        ...
        },
        {
          ...
          endDate: new Date(Date.UTC(2026, 6, 6)),
          amount: testPlan.price,
      ...
      expect(result.billingPeriods[0].startDate).toEqual(new Date(Date.UTC(2026, 4, 5)));
      expect(result.billingPeriods[0].endDate).toEqual(new Date(Date.UTC(2026, 5, 6)));

      expect(result.billingPeriods[1].status).toBe('PAID');
      expect(result.billingPeriods[1].paymentMethod).toBe('TRANSFER');
      expect(result.billingPeriods[1].startDate).toEqual(new Date(Date.UTC(2026, 5, 6)));
      expect(result.billingPeriods[1].endDate).toEqual(new Date(Date.UTC(2026, 6, 6)));

      expect(result.billingPeriods[2].status).toBe('PENDING');
      expect(result.billingPeriods[2].startDate).toEqual(new Date(Date.UTC(2026, 6, 6)));
      expect(result.billingPeriods[2].endDate).toEqual(new Date(Date.UTC(2026, 7, 6)));
    ...
    it('debería rechazar fecha de activación futura', () => {
      ...
      expect(() => {
        service.createSubscription({
          ...
          maxOverduePeriods: 2,
          registrationDate: new Date(),
          activationDate: new Date(Date.UTC(2026, 11, 31)),
          historicalPayments: [],
        });
          ...
          historicalPayments: [
            {
              ...
              endDate: new Date(Date.UTC(2026, 5, 6)),
              amount: testPlan.price,
              paidAt: new Date(Date.UTC(2027, 0, 1)),
              paymentMethod: 'CASH',
            },
          ],
        });
          ...
          historicalPayments: [
            {
              ...
              endDate: new Date(Date.UTC(2026, 4, 6)),
              amount: testPlan.price,
              paidAt: new Date(Date.UTC(2026, 4, 1)),
              paymentMethod: 'CASH',
            },
          ],
        });
          ...
          historicalPayments: [
            {
              periodLabel: 'Invalid',
              startDate: new Date(Date.UTC(2026, 5, 6)),
              endDate: new Date(Date.UTC(2026, 4, 5)),
              amount: testPlan.price,
              paidAt: new Date(Date.UTC(2026, 5, 1)),
              paymentMethod: 'CASH',
            },
          ],
        });
    ...
    it('debería permitir activationDate igual a hoy', () => {
      ...
      const result = service.createSubscription({
        ...
        maxOverduePeriods: 2,
        registrationDate: new Date(),
        activationDate: new Date(Date.UTC(2026, 6, 30)),
      });

      expect(result.billingPeriods.length).toBe(1);
      expect(result.billingPeriods[0].status).toBe('PENDING');
      expect(result.billingPeriods[0].startDate).toEqual(new Date(Date.UTC(2026, 6, 30)));
...
describe('Business Rules', () => {
  ...
  describe('getBillingPeriodRange', () => {
    it('debería calcular el período correcto cuando la fecha es después del día de corte', () => {
      ...
      expect(result.startDate.getUTCDate()).toBe(5);
      expect(result.startDate.getUTCMonth()).toBe(6);
      expect(result.endDate.getUTCDate()).toBe(5);
    ...
    it('debería calcular el período correcto cuando la fecha es antes del día de corte', () => {
      const referenceDate = new Date(Date.UTC(2026, 6, 3));
      const result = getBillingPeriodRange(referenceDate, 5);

      expect(result.startDate.getUTCDate()).toBe(5);
      expect(result.startDate.getUTCMonth()).toBe(5);
      expect(result.endDate.getUTCDate()).toBe(5);
      expect(result.endDate.getUTCMonth()).toBe(6);
    });
  });
});
